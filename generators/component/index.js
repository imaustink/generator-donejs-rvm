...
module.exports = BaseGenerator.extend({
  ...
  prompting: function () {
    ...
    this.prompt({
      name: 'name',
      message: 'What is the module name of your component (e.g. components/my-component)?',
      validate: utils.validateRequired,
      when: !this.options.name
    }).then(function (first) {
      let name = this.options.name = this.options.name || first.name

      this.isDoneComponent = this.options.name.indexOf('.component') !== -1
      this.options.name = name = name.replace('.component', '')

      const tag = _.startCase(name).replace(' ', '')
      const prompts = [{
        name: 'tag',
        message: 'A unique name for the component (e.g. MyComponent)',
        default: tag,
  ...
  writing: function () {
    const isDoneComponent = this.isDoneComponent
    const self = this
    const done = this.async()
    ...
    const fullPath = isRootComponent ? [folder] : [folder].concat(parts)
    ...
    if (isDoneComponent) {
      ...
      this.modletFiles.forEach(function (name) {
        const target = name.replace('component', options.name).replace('modlet/', '')
        self.fs.copyTpl(
      ...
      const mainTests = this.destinationPath(path.join(folder, 'test.js'))
