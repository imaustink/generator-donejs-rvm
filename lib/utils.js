...
exports.getPkgOrBail = function (env, done) {
  const pkgFile = env.destinationPath('package.json')
  const pkg = env.fs.readJSON(pkgFile, false)

  if (pkg === false) {
    const error = new Error('Expected to find a package.json file at ' + pkgFile +
                            ' but did not')
    done(error)
    return false
  }
...
exports.templatePath = function (subPath) {
  return function (target) {
    const override = path.join(this.destinationPath(), subPath, target)
...
exports.getKeywords = function (kind, keywords) {
  ...
  const kindKeyword = npmKeywords[kind]
